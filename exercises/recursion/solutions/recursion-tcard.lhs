...
>                | otherwise   = xs
> -- @ notation is not introduced yet
...
>   maximum' :: Ord a => a -> [a] -> a
>   maximum' m []     = m
>   maximum' m (x:xs) | x > m     = maximum' x xs
>                     | otherwise = maximum' m xs
...
>     minimum' :: Ord a => a -> [a] -> a
>     minimum' m []     = m
>     minimum' m (x:xs) | x < m     = minimum' x xs
>                       | otherwise = minimum' m xs
...
>     lines' acc (c:cs) | c == '\n' = reverse acc : lines' "" cs
>                       | otherwise = lines' (c : acc) cs
...
>     words' acc []     | null acc  = []
>                       | otherwise = reverse acc : []
>     words' acc (c:cs) | isSpace c = if null acc
>                                       then words' acc cs
>                                       else reverse acc : words' "" cs
>                       | otherwise = words' (c : acc) cs
...
> splitAt n xs | n <= 0 = ([], xs)
> splitAt _ []          = ([], [])
> splitAt n (x:xs)      = let (bs,es) = splitAt (n - 1) xs in (x : bs, es)
...
> span :: (a -> Bool) -> [a] -> ([a], [a])
> span _ [] = ([], [])
> span p xs = let x = head xs
>             in  if p x
>                   then let (bs, es) = span p (tail xs) in (x : bs, es)
>                   else ([], xs)
> -- @ notation is not introduced yet
...
> break :: (a -> Bool) -> [a] -> ([a], [a])
> break _ [] = ([], [])
> break p xs = let x = head xs
>              in  if p x
>                    then ([], xs)
>                    else let (bs, es) = break p (tail xs) in (x : bs, es)
> -- @ notation is not introduced yet
...
> unzip :: [(a, b)] -> ([a], [b])
> unzip []         = ([], [])
> unzip ((x,y):rest) = let (xs,ys) = unzip rest in (x : xs, y : ys)
...
> unzip3 :: [(a, b, c)] -> ([a], [b], [c])
> unzip3 []             = ([], [], [])
> unzip3 ((x,y,z):rest) = let (xs,ys,zs) = unzip3 rest
>                         in  (x : xs, y : ys, z : zs)
...
> scanr :: (a -> b -> b) -> b -> [a] -> [b]
> scanr _ v []     = [v]
> scanr f v (x:xs) = let vs = scanr f v xs in f x (head vs) : vs
